...
function CustomerModal({ customer, onClose, onSaved }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl border border-gray-200 flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          {/* Name — accepts English and Tamil */}
          <div>
          ...
          {/* Address — accepts English and Tamil */}
          <div>
...
export default function Customers() {
  ...
  const loadPending = useCallback(async () => {
    try {
      ...
      const map = {};
      for (const c of res.data.data) map[c._id] = c.pendingAmount || 0;
      setPendingMap(map);
    } catch (_) {}
  }, []);
    ...
    <div className="space-y-6">
      ...
      <div className="card p-4">
        <form onSubmit={handleSearch} className="flex gap-3 items-end flex-wrap">
          ...
          <button type="button" className="btn-secondary" onClick={() => { load(); loadPending(); }} title="Refresh">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                {['#', 'Name / பெயர்', 'Mobile', 'Address / முகவரி', 'Pending', ''].map(h => (
